...
def parse_games_array(arr)
  games = []

  arr.each do |scorecard|
    team_1, score_1, _, score_2, team_2 = scorecard.split(' ')
    games << { "#{team_1}": score_1, "#{team_2}": score_2 }
  end
  
  games    
end

def get_team_names(games)
  team_names = []
  games.each { |game| team_names.append(*game.keys) }
  team_names.uniq.sort
end

def initialize_score_table(games)
  teams = get_team_names(games)
  score_table = Hash.new()
  teams.each do |team| 
    score_table[team] = { points: 0, goals: 0, goaldiff: 0 }
  end
  score_table
end

def update_score_table(game, score_table)
  
end

def tournament_scores(arr)
	games = parse_games_array(arr)
  score_table = initialize_score_table(games)

  games.each do |game|
    update_score_table(game, score_table)
  end
...
#p tournament_scores(["A 4 - 0 B", "C 2 - 1 D", "B 1 - 0 C", "D 3 - 2 A", "A 1 - 3 C", "B 2 - 1 D"]) ➞ [ [ "C", 6, 5, 2 ], [ "B", 6, 3, -2 ], [ "A", 3, 7, 1 ], [ "D", 3, 5, -1 ] ])
# # Final order is C, B, A, D (C and B have same points, but C has more scored goals than B; A and D have same points but A has more scored goals than D).

#p tournament_scores(["A 2 - 1 B", "C 3 - 0 D", "B 1 - 1 C", "D 1 - 0 A", "A 3 - 0 C", "B 2 - 4 D"]) ➞ [ [ "A", 6, 5, 3 ], [ "D", 6, 5, 0 ], [ "C", 4, 4, 0 ], [ "B", 1, 4, -3 ] ])
# # Final order is A, D, C, B (A and D have same points and same number of scored goals, but A has a greater goals difference than D).
