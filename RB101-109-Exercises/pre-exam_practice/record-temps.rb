## https://edabit.com/challenge/iQowMxboTXmusqmjw
...
def record_temps(records, currentWeek)
	updated_records = records.clone

  0.upto(6) do |dayNum|
    if currentWeek[dayNum][LO_TEMP] < updated_records[dayNum][LO_TEMP]
      updated_records[dayNum][LO_TEMP] = currentWeek[dayNum][LO_TEMP]
    end
    if currentWeek[dayNum][HI_TEMP] > updated_records[dayNum][HI_TEMP]
      updated_records[dayNum][HI_TEMP] = currentWeek[dayNum][HI_TEMP]
    end
  end
